    ...
    <body>
		<script type="text/javaScript">
			...
			function inscat_mapper_ui()
			{
				...
				createcat_map.setRequestHeader("import_map_name", document.getElementById("imptxt").value);
				...
				createcat_map.setRequestHeader("source_xml_file_name", document.getElementById("srcfile").value);
				createcat_map.setRequestHeader("target_xml_file_name", document.getElementById("trgfile").value);
		...
		var opendstFile = function(event)
		{
				...
				function parsetgtXML(xmlNode)
				{
					...
					var i;
					rum++;
					elearr1.push('<span id="d'+rum+'">' + '<ul id="myUL"><li><span class="caret1">' + node.nodeName + '</span><div class="rect" ondrop="drop(event)" ondragover="allowDrop(event)" id="dstid'+rum+'"></div><div class="removebtncls"><a href="#" onclick="removeele(dstid'+rum+')"><img src="001-delete.png" style="height:20px"></a></div>'+'<ul class="nested1">');
				  	for(i=0; childNodes && i<childNodes.length; i++)
				  	{
						if(childNodes[i].children && childNodes[i].children.length > 0)
						...
						{	rum++;
							elearr1.push('<li><span id="d'+rum+'">'+childNodes[i].nodeName + '</li></span><div class="rect"  ondrop="drop(event)" ondragover="allowDrop(event)" id="dstid'+rum+'"></div><div class="removebtncls"><a href="#" onclick="removeele(dstid'+rum+')"><img src="001-delete.png" style="height:20px"></a></div></span></li>');
						}
				  	}
				 	elearr1.push('</ul>');
				}			
        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-lg-12">
                            <h1 class="page-header">Create Product XML PARSING and MAPPING</h1>
                            
                           
					<span class="but">
				<!--<input type="submit" style="width:5%" value="SEARCH" onclick="SEARCHMAP()"/>-->
				<input type="submit" style="width:7%" value="Edit" onclick="EDITMAP()"/>
		...
		<td><input type="text" class="form-control" id="imptxt" name='Import Map Name' placeholder='Your Import Map Name'/></td>
		</tr>
		<tr>
		<td><br></td>
		<td><br></td>
		...
		<td><input type='file' name="filename" id="srcfile" onchange="opensrcFile(event)"/></td>
		</tr>
		<tr>
		<td><br></td>
		<td><br></td>
		...
		<td><input type='file' name="filename" id="trgfile" onchange="opendstFile(event)"/></td>
		</tr>
		</table>
		</center>
		<div  class "rownew">
